...
<?php
    ...
    if(isset($_POST["themvaodatban"])){
        ...
        if(!isset($_SESSION["datban"]) || $_SESSION["datban"] == null){
            // mua 1 lần số lượn bằng 1
            $mang_giohang[$id]["so_luong1"] = 1;
            $_SESSION["datban"][$id] = $mang_giohang[$id];  
        }
        else{
            // kiểm tra sản phẩm có trong giỏ hàng chưa nếu có rồi thì +1
            if(array_key_exists($id, $_SESSION["datban"])){
                $_SESSION["datban"][$id]["so_luong1"] +=1;
            }else{
                $mang_giohang[$id]["so_luong1"] = 1;
                $_SESSION["datban"][$id] = $mang_giohang[$id];
            }
